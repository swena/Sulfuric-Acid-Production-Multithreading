/* MANUFACTURING PLANT : Manufacture of Sulphuric Acid 
                        - 5 stage process 
		        - Concepts used : Threads,Mutex,Signal Handlers 
...
struct machine_content machine[total_threads],*a[total_threads];
...
void main (int argc, char* argv[]) 	// used command-line arguments : to give the input as the 'quantity of acid' needed to be produced.
{
	...
	printf("Machine No.\t1lt incr temp by\tMax temp under which Machine works\tTemp Decrs'd in Cooling Machine\n\n");
	printf("    1\t\t\t20\t\t\t\t500\t\t\t\t\t20\n");
	printf("    2\t\t\t10\t\t\t\t55 \t\t\t\t\t10\n");
	printf("    3\t\t\t30\t\t\t\t575\t\t\t\t\t30\n");
	printf("    4\t\t\t5 \t\t\t\t39 \t\t\t\t\t5 \n");
...
{	
	...
	{
		printf("\nEnter temp of Machine[1] :");
		scanf("%d",&(my_data->temperature));
	}
	
	

	while(my_data->temperature>500)	// since prescribed condition on 1st machine is that it works only if its temp <=500
	{
		...
		printf("Present temp of this machine= %d\n",my_data->temperature);
	}
...
void *func_for_other_threads (void *ptr)			// machine[n]'s corresponding function, where n= 1 to 4
{
	...
	switch(taskid)					
	{
		case 1:
			if(num_of_times==0)	// i.e while 1st litre production, the initialization of machine's temperature takes place
			{
			printf("\nEnter temp of Machine[2] :");
			scanf("%d",&my_data->temperature);
			}
	
			while(my_data->temperature>55)
			{
		...
		case 3:				
		{
			...
			if(num_of_times==0)	// i.e while 1st litre production, the initialization of machine's temperature takes place
			{
					printf("\nEnter temp of Machine[4] :");
		scanf("%d",&my_data->temperature);
			}

			while(my_data->temperature>39)
			{
